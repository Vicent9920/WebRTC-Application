...
function Basketball() {
	...
	this.sound = false;
	this.state = 'menu';
    this.menuText = new AnimatedText("Click / Tap to Play!", 640 / 2, 960 / 2 + 50, 40, 0.01);
    this.overText = new AnimatedText("Click / Tap to continue!", 640 / 2, 800, 40, 0.01);
    this.flashText = [];
    this.scored = 0;
    this.totalBalls = 3;
    this.round = 1;
		...
		if(this.state == 'play') {
			...
			for(var i = 0; i < this.balls.length; i++) {
				...
				if(ball.y > 960) {
                    ...
                    if(!ball.scored) {
                        this.flashText.push(new FlashText("Miss!"));
                        if(++this.missed >= 3) {
                            this.state = 'over';
                        }
                    }
	...
	this.draw = function(ctx) {
		...
		if(this.state == 'menu') {
			...
			this.drawText(ctx, '(?) Click / Tap to throw a ball. Game over if you miss 3 times.', 640 / 2, 940, 26);
			ctx.textAlign = 'left';
		}
		if(this.state == 'play') {
			for(var i = 0; i < this.hoops.length; i++) {
            ...
            ctx.textAlign = "left";
            this.drawText(ctx, "Score: " + this.score, 50, 70, 40);
            this.drawText(ctx, "Missed: " + this.missed, 460, 70, 40);
			for(var i = 0; i < this.texts.length; i++) {
				var text = this.texts[i];
				text.draw(ctx);
			}
		...
		if(this.state == 'over'){
			...
			this.drawText(ctx, 'Score: ' + this.displayScore, 640 / 2, 400, 50);
            if(this.storage) {
                this.drawText(ctx, "High Score: " + localStorage.score, 640 / 2, 500, 50);
            }
